...
export default async function SubrubroPage({
  ...
  params: Promise<{ rubro: string; subrubro: string }>;
  searchParams?: Promise<Record<string, string | string[] | undefined>>;
}) {
  const { rubro, subrubro } = await params;
  ...
  function pageHref(nextPage: number) {
    ...
    return `/rubros/${rubro}/${subrubro}?${q.toString()}`;
  }
    ...
    <main>
      <Container>
        ...
        <div className="text-sm text-slate-600">
          ...
          <span className="mx-2 text-slate-400">/</span>
          <Link href={`/rubros/${rubro}`} className="underline hover:text-slate-900">{rubro}</Link>
          <span className="mx-2 text-slate-400">/</span>
        ...
        <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">Subrubro: {subrubro}</h1>
        ...
        <div className="mt-4 grid gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2">
            <ProviderFiltersBar
              basePath={`/rubros/${rubro}/${subrubro}`}
